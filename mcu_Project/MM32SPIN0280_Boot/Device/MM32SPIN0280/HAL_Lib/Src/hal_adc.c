...
/// @retval None.
////////////////////////////////////////////////////////////////////////////////
void ADC_RegularChannelConfig(ADC_TypeDef* adc, ADCCHANNEL_TypeDef channel, u8 rank, u32 sample_time)
{
    u32 tempchan;
    ...
    if ((1 << channel) & ADC_CHSR_CHT)
        ADC_TempSensorVrefintCmd(ENABLE);
    else if ((1 << channel) & ADC_CHSR_CHV)
        ADC_TempSensorVrefintCmd(ENABLE);
}
////////////////////////////////////////////////////////////////////////////////
/// @brief  Enable the selected ADC channel and configure its sample time. Please
...
void ADC_AnychanChannelConfig(ADC_TypeDef* adc, ADCCHANNEL_TypeDef channel, uint8_t anychan, uint32_t sample_time)
{
    ...
    if((ADC1 == adc) && ((ADC_Channel_VoltReference == channel) || (ADC_Channel_TempSensor == channel))) {
        ADC_TempSensorVrefintCmd(ENABLE);
    }
}
...
FlagStatus ADC_GetFlagStatus(ADC_TypeDef* adc, ADCFLAG_TypeDef adc_flag)
{
    ...
    if (ADC_FLAG_ENDOFCONVSEQUENCE == adc_flag) {
        ret =  ((adc->ADSTA&= ADC_SR_EOSIF) ? SET : RESET);
    }
    else if (ADC_FLAG_ANALOGWATCHDOG == adc_flag) {
        ret =  ((adc->ADSTA&= ADC_SR_AWDIF) ? SET : RESET);
    }
    else if (ADC_FLAG_ENDOFCONVSINGLE == adc_flag) {
        ret =  ((adc->SREXT &= ADC_SREXT_EOCIF) ? SET : RESET);
    }
    else if (ADC_FLAG_ENDOFCONVSAMPLE == adc_flag) {
        ret =  ((adc->SREXT &= ADC_SREXT_EOSMPIF) ? SET : RESET);
    }
    else if (ADC_FLAG_INJENDOFCONVSEQUENCE == adc_flag) {
        ret =  ((adc->SREXT &= ADC_SREXT_JEOSIF) ? SET : RESET);
    }
    else if (ADC_FLAG_INJENDOFCONVSINGLE == adc_flag) {
        ret =  ((adc->SREXT &= ADC_SREXT_JEOCIF) ? SET : RESET);
    }
    else if (ADC_FLAG_INJENDOFCONVSAMPLE == adc_flag) {
        ret =  ((adc->SREXT &= ADC_SREXT_JEOSMPIF) ? SET : RESET);
    }
    return ret;
}
...
{
    if (ADC_FLAG_ENDOFCONVSEQUENCE == adc_flag) {
        adc->ADSTA= ADC_SR_EOSIF;
    }
    else if (ADC_FLAG_ANALOGWATCHDOG == adc_flag) {
        adc->ADSTA= ADC_SR_AWDIF;
    }
    else if (ADC_FLAG_ENDOFCONVSINGLE == adc_flag) {
        adc->SREXT = ADC_SREXT_EOCIF;
    }
    else if (ADC_FLAG_ENDOFCONVSAMPLE == adc_flag) {
        adc->SREXT = ADC_SREXT_EOSMPIF;
    }
    else if (ADC_FLAG_INJENDOFCONVSINGLE == adc_flag) {
        adc->SREXT = ADC_SREXT_JEOCIF;
    }
    else if (ADC_FLAG_INJENDOFCONVSAMPLE == adc_flag) {
        adc->SREXT = ADC_SREXT_JEOSMPIF;
    }
    else if (ADC_FLAG_INJENDOFCONVSEQUENCE == adc_flag) {
        adc->SREXT = ADC_SREXT_JEOSIF;
    }
}
...
{
    (state) ? (adc->ANYCR |= ADC_ANY_CR_CHANY_MDEN) : (adc->ANYCR &= ~ADC_ANY_CR_CHANY_MDEN);
}
...
{
    adc->ANYCR &= ~(ADC_ANY_CR_JCEN | (ADC_ANY_CR_JTRGSEL) | \
                    ADC_ANY_CR_JTRGSHIFT_512 | ADC_ANY_CR_JTRGEN);
    adc->ANYCR |= (ADC_ANY_CR_JCEN | (source & ADC_ANY_CR_JTRGSEL) | \
...
////////////////////////////////////////////////////////////////////////////////
/// @brief  ADC sampling value calibration.
/// @param  adc  :   select the ADC peripheral.
/// @retval None.
////////////////////////////////////////////////////////////////////////////////
void ADC_Calibration(ADC_TypeDef* adc)
{
    adc->ANYCR     |= ADC_ANY_CR_ADCAL;
    adc->ADCFG     |= ADC_CFGR_ADEN;
    while(!(adc->SREXT & (ADC_SREXT_EOCALIF))) {
        ;
    }
