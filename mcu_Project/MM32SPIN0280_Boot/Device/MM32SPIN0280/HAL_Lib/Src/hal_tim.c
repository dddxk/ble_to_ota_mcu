...
{
    MODIFY_REG(tim->CCER, (TIM_CCER_CC1P | TIM_CCER_CC1NP) << 0, (pol << 0));
}
...
{
    MODIFY_REG(tim->CCER, (TIM_CCER_CC1P | TIM_CCER_CC1NP) << 4, (pol << 4));
}
...
{
    MODIFY_REG(tim->CCER, (TIM_CCER_CC1P | TIM_CCER_CC1NP) << 8, (pol << 8));
}
...
{
    MODIFY_REG(tim->CCER, (TIM_CCER_CC1P | TIM_CCER_CC1NP) << 12, (pol << 12));
}
...
{
    MODIFY_REG(tim->CCMR3, TIM_CCMR3_OC5M, init_struct->TIM_OCMode);

    WRITE_REG(tim->CCR5, init_struct->TIM_Pulse);
}
...
{
    (state) ? SET_BIT(tim->BKINF, TIM_BKINF_BKINFE) : CLEAR_BIT(tim->BKINF, TIM_BKINF_BKINFE);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief  Configure the TIM Break input filter.
/// @param  tim: select the TIM peripheral.
...
/// @retval None.
////////////////////////////////////////////////////////////////////////////////
void TIM_BreakInputFilterConfigure(TIM_TypeDef* tim, u16 filter)
{
    MODIFY_REG(tim->BKINF, TIM_BKINF_BKINF, filter);
}
