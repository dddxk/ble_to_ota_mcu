...
vu8 sSendBuf[RECVBUFLENGTH] = "0123456789ABCDEFGHIJKLMN";
vu8 sRecvBuf[RECVBUFLENGTH];
vu16 RecvLen;
vu16 SendLen;
    ...
    if (UART_GetITStatus(UARTx, UART_IT_TXIEN) != RESET)
    {
        ...
        if (pUARTxSendStruct->UART_SendComplete == false)
        {
            ...
            else
            {
                ...
                {
                    UART_SendData(UARTx, *(vu8 *)(pUARTxSendStruct->UART_SendBufferAddress + pUARTxSendStruct->UART_SendRealCnt));
